        ...
        try {
            ...
            if (type) {
                ...
                if (!/^[a-zA-Z0-9_-]+$/.test(type)) {
                    throw new Error(`Invalid script type: ${type}. Only alphanumeric characters, underscores, and hyphens are allowed.`);
                }
                // Load from an external file
                const module = await import(`../scripts/${type}.js`);
                ComponentClass = module.default;
            } else {
                throw new Error('Script configuration must have a type. Note: inline "source" scripts are no longer supported for security reasons.');
            }
