...
export function createGameLoop(renderer, canvas) {
    ...
    return function gameLoop(time) {
        ...
        if (TimeManager.isPlaying()) {
            ...
            // PERF: In-place removal of destroyed objects.
            // Using a reverse loop with splice is more memory-efficient than `filter()`
            // because it modifies the array in-place, avoiding the creation of a new
            // array and reducing garbage collection pressure on each frame.
            // This is done in reverse to avoid issues with array indexing when removing items.
